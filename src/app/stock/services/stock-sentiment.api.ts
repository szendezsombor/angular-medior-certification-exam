...
export class StockSentimentApi {
  ...
  getStockSentiment(stockSymbol: string): Observable<StockSentimentModel[]> {
    ...
    return this.http.get<StockSentimentResponseModel>('https://finnhub.io/api/v1/stock/insider-sentiment', {params})
      .pipe(
        pluck('data'),
        map((stockSentiments: StockSentimentModel[]) => stockSentiments.map((stockSentiment: StockSentimentModel) => {
